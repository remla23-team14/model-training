...
def test_negated_words_data_slice(dataset, corpus, model, classifier):
    ...
    negated_indices = [
        i
        for i, review in enumerate(corpus)
        if any(word in review for word in negated_words)
    ]
    ...
    assert (
        negated_metrics[0] >= complete_metrics[0] * 0.9
    )  # Accuracy threshold (90% of complete accuracy)
    assert (
        negated_metrics[1] >= complete_metrics[1] * 0.9
    )  # Precision threshold (90% of complete precision)
    assert (
        negated_metrics[2] >= complete_metrics[2] * 0.8
    )  # Recall threshold (90% of complete recall)
